...
export const useFactoryStore = create<FactoryState>((set, get) => ({
    ...
    setMachineCount: (blockId, count) => {
        ...
        if (block instanceof ProductionBlock) {
            block.machineCount = Math.max(1, count);
            set(state => ({ version: state.version + 1 }));
            debouncedSolve(get);
        }
    },
    ...
    runSolver: () => {
        ...
        console.log(`[SOLVER] Running iteration on ${Object.keys(layoutDTO.blocks).length} blocks... Mode: ${autoSolveEnabled ? 'AUTO' : 'MANUAL'}`);
                ...
                if (block.type === 'block') {
                    ...
                    if (prodBlock.recipeId) {
                        ...
                        if (recipe && machine && recipe.outputs[0]) {
                            const yieldMult = recipe.category === 'Gathering' ? (prodBlock.sourceYield ?? 1.0) : 1.0;
                            const mainOutput = recipe.outputs[0];
                            const ratePerMachine = (mainOutput.amount * machine.speed / recipe.craftingTime) * yieldMult;
                            const requiredRate = prodBlock.requested?.[mainOutput.itemId] || 0;
                            if (ratePerMachine > 0) {
