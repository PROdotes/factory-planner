...
export const BlockCard = memo(({ block, scale }: Props) => {
    ...
    const { moveBlock, selectBlock, selectedBlockId, removeBlock, updateBlockName, setRequest, setYield, setMachineCount, version } = useFactoryStore();
    ...
    const isDimmed = highlightSet.blocks.size > 0 && !highlightSet.blocks.has(block.id);

    // Determine which items to highlight on THIS block
    const connectedInputItems = highlightSet.connectedInputs.get(block.id);
    const isOutputHighlight = (itemId: string) => isSelected && highlightSet.outputItems.has(itemId);
    const isInputHighlight = (itemId: string) => connectedInputItems?.has(itemId) ?? false;
    ...
    const recipe = block instanceof ProductionBlock && block.recipeId ? recipes[block.recipeId] : null;
    ...
    if (block instanceof ProductionBlock && recipe && machine && mainOutput) {
        ...
        const ratePerMachine = (mainOutput.amount * machine.speed / recipe.craftingTime) * yieldMult;
        const targetRate = block.requested[mainOutput.itemId] || block.output[mainOutput.itemId] || 0;
    ...
    const inputItems = block.type === 'sink'
    ...
    const outputItems = (recipe?.outputs.map(o => ({
        itemId: o.itemId,
    ...
    const workingPowerMW = machine && requiredMachineCount > 0
    ...
    const commitOutputRate = (val: number) => {
        if (!isNaN(val) && val > 0 && mainOutput) {
            ...
            if (recipe?.category === 'Gathering') {
                ...
                if (machine) {
                    const ratePerVein = (mainOutput.amount * machine.speed / recipe.craftingTime);
                    if (ratePerVein > 0) {
    ...
    return (
        <div
            className={`block-card ${block.type} ${isDragging ? 'dragging' : ''} ${isSelected ? 'selected' : ''} ${isDimmed ? 'dimmed' : ''} ${statusClass}`}
            style={{
        ...
        >
            ...
            {ports.map((port: PortDescriptor, i: number) => {
                const isHighlighted = port.side === 'right'
                    ? isOutputHighlight(port.itemId)
                    : isInputHighlight(port.itemId);
                return (
                    <div
                        key={`${port.side}-${port.itemId}-${i}`}
                        className={`port ${port.side} ${isHighlighted ? 'highlighted' : ''}`}
                        style={{
                            top: `${port.y}px`,
                            left: port.side === 'left' ? `-${FLOW_CONFIG.PORT_RADIUS}px` : 'auto',
                            right: port.side === 'right' ? `-${FLOW_CONFIG.PORT_RADIUS}px` : 'auto',
                        }}
                        title={items[port.itemId]?.name || port.itemId}
                        onMouseDown={(e) => {
                            e.stopPropagation();
                            window.dispatchEvent(new CustomEvent('port-drag-start', {
                                detail: { blockId: block.id, itemId: port.itemId, side: port.side, x: e.clientX, y: e.clientY }
                            }));
                        }}
                        onMouseUp={() => {
                            window.dispatchEvent(new CustomEvent('port-drag-end', {
                                detail: { blockId: block.id, itemId: port.itemId, side: port.side }
                            }));
                        }}
                    >
                        <ItemIcon itemId={port.itemId} size={FLOW_CONFIG.PORT_RADIUS * 1.4} />
                    </div>
                )
            })}
                    ...
                    <div className="zoom-out-view">
                        <div className="main-icon">
                            {mainOutput ? (
                                ...
                                <ItemIcon itemId={block.type === 'sink' ? 'storage-container' : 'iron-ore'} size={48} />
                            )}
                        </div>
                        {requiredMachineCount > 0 && (
                            <div className="machine-badge">×{Math.ceil(requiredMachineCount)}</div>
                        )}
                    </div>
                ) : (
                    /* ===== ZOOMED IN VIEW ===== */
                            ...
                            <div className="controls-row">
                                ...
                                {recipe?.category === 'Gathering' ? (
                                    ...
                                    <div
                                        ...
                                        title={recipe.machineId.includes('pump') || recipe.machineId.includes('extractor') ? "Yield Multiplier (%)" : "Veins Count"}
                                    >
                                        <div className="field-value-row">
                                            {isEditingYield ? (
                                                ...
                                                <span className="control-value">×{block.sourceYield.toFixed(1)}</span>
                                            )}
                                        </div>
                                        <span className="field-label">{recipe.machineId.includes('pump') || recipe.machineId.includes('extractor') ? 'YIELD %' : 'VEINS'}</span>
                                    </div>
                                ) : (
                                    ...
                                    <div
                                        ...
                                        onClick={(e) => {
                                            ...
                                            if (!wasDragged && !isEditingCount) {
                                                const currentCount = autoSolveEnabled ? requiredMachineCount : (block instanceof ProductionBlock ? block.machineCount : requiredMachineCount);
                                            ...
                                            {isEditingCount ? (
                                                ...
                                                <span className="control-value">
                                                    ×{autoSolveEnabled
                                                        ...
                                                        : (block instanceof ProductionBlock ? block.machineCount.toFixed(1) : requiredMachineCount.toFixed(1))}
                                                </span>
                                            )}
                                        </div>
                                ...
                                <div
                                    ...
                                    onWheel={(e) => {
                                        ...
                                        const next = e.deltaY < 0 ? current + delta : current - delta;
                                ...
                                >
                                    <div className="field-value-row">
                                        {isEditingRate ? (
                                            ...
                                            <span className="control-value">{targetRateUnitValue.toFixed(1)}{rateLabel}</span>
                                        )}
                                    </div>
                                    ...
                                    >
                                        <div className="field-value-row">
                                            {isEditingCount ? (
                                                ...
                                                <span className="control-value">×{autoSolveEnabled ? requiredMachineCount.toFixed(1) : (block as ProductionBlock).machineCount}</span>
                                            )}
                                        </div>
                            ...
                            {outputItems.length > 0 && (
                                <div className="io-group outputs">
                                    {outputItems.map(item => {
                                        ...
                                        return (
                                            <div key={item.itemId} className={`io-row output ${isItemHighlighted ? 'highlighted' : ''}`} title={item.name}>
                                                <span className="io-rates">
                                                    <span>{formatRate(item.actual, isPerMin)}</span>
                                                    <span className="rate-sep">/</span>
                                                    <span className="rate-target">{formatRate(item.target, isPerMin)}</span>
                                                </span>
                            ...
                            {inputItems.length > 0 && (
                                <div className="io-group inputs">
                                    {inputItems.map(item => {
                                        ...
                                        return (
                                            <div key={item.itemId} className={`io-row ${isItemHighlighted ? 'highlighted' : ''}`} title={item.name}>
                                                <span className="io-rates">
                                                    <span className={sat < 0.999 ? 'rate-bad' : ''}>{formatRate(item.actual, isPerMin)}</span>
                                                    <span className="rate-sep">/</span>
                                                    <span className="rate-target">{formatRate(item.target, isPerMin)}</span>
                                                </span>
                                                <div className="io-bar">
            ...
            <div className="block-footer">
                <div
                    ...
                    style={{
                        background: `linear-gradient(to right, ${getBarColor(block.satisfaction)} ${block.satisfaction * 100}%, transparent ${block.satisfaction * 100}%)`
                    }}
                />
                <span className="efficiency-pct">{(block.satisfaction * 100).toFixed(0)}%</span>

                {workingPowerMW > 0 && (
                    <div className="power-info">
                        <span className="power-working">{workingPowerMW.toFixed(1)} MW</span>
                    </div>
                )}
            </div>
        </div>
