...
export interface DragSpawnPayload {
    type: 'recipe' | 'sink' | 'splitter' | 'merger';
    recipeId?: string;
    label: string;
}
...
declare global {
    interface WindowEventMap {
        'spawn-drag-start': CustomEvent<{ clientX: number, clientY: number, payload: DragSpawnPayload }>;
    }
}

export function useDragToSpawn(clientToWorld: (x: number, y: number) => { x: number, y: number }) {
    ...
    useEffect(() => {
        const startDrag = (e: CustomEvent<{ clientX: number, clientY: number, payload: DragSpawnPayload }>) => {
        ...
        return () => window.removeEventListener('spawn-drag-start', startDrag as any);
